...
class FeatAgent:
	...
	def feat_q_update(self, state, AV_state, action, reward, next_state, next_state_possible_actions, done, features):
		...
		print("qval ", qval)
		a = self.get_value(next_state, next_state_possible_actions)
		print("a ", a)

		difference = (reward + self.discount * self.get_value(next_state, next_state_possible_actions)) - qval

		for i in range(self.feat_weights):
...
default_reward	= -1
road_rewards 	= [-7 for i in range(4*gridH)] 
...
while(True):
	...
	action = agent.get_action(state, possible_actions, predicted_features) # for feature-based
	...
	agent.feat_q_update(state, AV_state, action, reward, next_state, next_state_possible_actions, done, features)
	state = next_state
	time.sleep(3)
